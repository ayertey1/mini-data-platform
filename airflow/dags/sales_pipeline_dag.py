...
def download_file(**context):
    ...
    try:
        ...
        if not files_to_process:
            logger.info(
                "No files to process - this is normal for cleanup runs when no new files are present"
            )
...
def validate_clean(**context):
    ...
    try:
        ...
        files_to_process = context["task_instance"].xcom_pull(
            key="files_to_process", task_ids="download_file"
        )
        ...
        for file_key in files_to_process:
            try:
                ...
                logger.info(
                    f"File {file_key} loaded with {len(df)} rows and columns: {df.columns.tolist()}"
                )
        ...
        logger.info(
            f"Validation completed: {len(processed_files)} processed, {len(rejected_files)} rejected"
        )
...
def insert_postgres(**context):
    ...
    try:
        ...
        if not processed_files or len(processed_files) == 0:
            ...
            return {"rows_inserted": 0, "message": "No processed files to insert"}
...
def move_file(**context):
    ...
    try:
        ...
        total_rows_inserted = context["task_instance"].xcom_pull(
            key="total_rows_inserted", task_ids="insert_postgres"
        )
        ...
        logger.info(
            f"File movement completed: {files_moved} files moved, {total_rows_inserted or 0} total rows processed"
        )
...
(
    start_cleanup
    >> get_config_cleanup
    >> init_clients_cleanup
    >> download_file_cleanup
    >> validate_clean_cleanup
    >> insert_postgres_cleanup
    >> move_file_cleanup
    >> end_cleanup
)
